...
### De Brujin Notation 
In De Brujin Notation, variables in lambda calculus are represented by integers *n* that refer to the index of thier binder while the lambda abstractions are represented by λ.e. This way the lambda abstractions remain nameless. So if we have given the following expression in lambda calculus: 
```
λx.x 
```
it becomes
